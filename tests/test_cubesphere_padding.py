...
"""
Tests for cubesphere padding functionality.
...
def test_cubesphere_pad(device):
    """Test cubesphere padding with shape and gradient validation.
    ...
    out = cubesphere.pad(x, padding=padding)
    out.mean().backward()
...
@pytest.mark.parametrize("device", ["cpu", "cuda"])
def test_cubesphere_reorder(device):
    """Test cubesphere reorder roundtrip.
    ...
    out = cubesphere.reorder(data, src=e3sm, dest=xy)
    out = cubesphere.reorder(out, src=xy, dest=e3sm)
