...
def healpix_pad(x: torch.Tensor, padding: int, enable_nhwc: bool = False) -> torch.Tensor:
    ...
    :param data: The input tensor of shape [N, F, H, W] where each face is to be padded in its HPX context
    :return: The padded tensor where each face's height and width are increased by 2*p
    """
    if padding == 0:
        return x

    padder = HEALPixPadding(padding, enable_nhwc)
    return padder.pad(x)
...
class HEALPixPadding(th.nn.Module):
    ...
    def pad(self, data: th.Tensor) -> th.Tensor:
        ...
        f00, f01, f02, f03, f04, f05, f06, f07, f08, f09, f10, f11 = [
            torch.squeeze(x, dim=1) for x in th.split(tensor=data, split_size_or_sections=1, dim=1)
        ]
